...
export const recordingViews = functions.https.onRequest(
    ...
    if (request.method === "POST") {
      ...
      For bonus points:
        ...
        3. Send a 400 response and return if the request is invalid
            response.status(400).send(); ✔️
        */

        //Validate that viewerId and recordingId exist and are non-empty strings
        if (typeof viewerId !== "string" || typeof recordingId !== "string") {
          response.status(400).send("viewerId & recordingId must be strings");
          return;
        }
        if (viewerId.length < 1 || recordingId.length < 1) {
          response.status(400).send("viewerId & recordingId must be non-empty");
          return;
        }
